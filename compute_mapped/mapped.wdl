task samtools_mapped {
    ...
    command <<<
        ...
        grep "chrX" ${SID}_aligned_chr_info.txt|awk -v tot=$Total  -v name=${SID} '{print "Sample""\t""%chrX""\n"name"\t"($3/tot)*100}' >chrX.txt
        grep "chrY" ${SID}_aligned_chr_info.txt|awk -v tot=$Total '{print "%chrY""\n"($3/tot)*100}' >chrY.txt
        grep "chrM" ${SID}_aligned_chr_info.txt|awk -v tot=$Total '{print "%chrM""\n"($3/tot)*100}' >chrM.txt
        grep "chr" ${SID}_aligned_chr_info.txt|grep -v "chrX\|chrY\|chrM" |awk -v tot=$Total '{sum+=$3}END{print "%chrAuto""\n"(sum/tot)*100}' >chrAuto.txt
        grep -v "chr\|^*" ${SID}_aligned_chr_info.txt|awk -v tot=$Total '{sum+=$3}END{print "%contig""\n"(sum/tot)*100}' >contig.txt
